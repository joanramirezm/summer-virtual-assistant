...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Botón para grabar manual (además de la hotword)
voiceBtn.addEventListener('click', function() {
    if (isRecordingFull) {
        // Si ya está grabando, detener
        stopFullRecognition();
    } else {
        // Si no está grabando, iniciar directamente la grabación completa
        startFullRecognition();
    }
});
    ...
    <a href="{{ route('assistant.history') }}"
        class="w-full sm:w-1/2 bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium py-3 px-6 rounded-lg transition duration-200 flex items-center justify-center">
        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor"
            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
